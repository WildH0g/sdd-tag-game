...
- `{output_path}`: absolute path to {output*dir}/{item_name_slug}.json (slugify item_name: replace spaces with *, remove special chars)
...
python .gemini/skills/research/validate_json.py -f {fields_path} -j {output_path}
...
## Validation
After completing JSON output, run validation script to ensure complete field coverage:
python .gemini/skills/research/validate_json.py -f {project_dir}/fields.yaml -j {project_dir}/results/GitHub_Copilot.json
Task is complete only after validation passes.
```
